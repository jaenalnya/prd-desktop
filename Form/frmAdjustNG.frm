...
Begin VB.Form frmAdjustNG 
   Caption         =   "Data Adjustment NG"
   ClientHeight    =   5250
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   9435
   ...
   Begin lvButton.lvButtons_H cmdNew 
      ...
      Mode            =   0
      Value           =   0   'False
      Image           =   "frmAdjustNG.frx":44F0
      cBack           =   16777215
   End
End
Attribute VB_Name = "frmAdjustNG"
...
'AltLVBackground lvList, vbWhite, &H8000000F, frmAdjustNG
...
With SQLParser
    ...
    .Tables = "(select a.plant_mark,a.prod_machine_id,b.number as machine_no,b.name as machine_name, a.mkt_customer_id,c.name as customer_name, " & _
            ...
            "a.period_shift,a.period_hour,a.prod_ng_id,f.name as ng_name, a.adjust,a.qty,a.description,a.created_at,a.created_by,e.name as account_name " & _
            "from prod_adjust_ngs a " & _
            ...
            "left join prod_ngs f on a.prod_ng_id = f.id) as AdjustNG"
...
RS_ADJUSTNG.Open SQLParser.SQLStatement, CN, adOpenDynamic, adLockPessimistic


With RecordPage
    .Start RS_ADJUSTNG, 500
End With
...
On Error Resume Next
    srcAdjustment = lvList.SelectedItem.Index
    srcRecord = lvList.ListItems.Item(srcAdjustment).Text
    Call RefreshRecSum
End Sub
...
'RS_ADJUSTNG.AbsolutePosition = RecordPage.PageStart
RecordPage.PageEnd
With lvList
    .GridLines = True
    .View = lvwReport
    ...
    .ColumnHeaders.Add , , "Period_Hour"
    .ColumnHeaders.Add , , "NG_Name"
    .ColumnHeaders.Add , , "Adjust"
    .ColumnHeaders.Add , , "Qty"
    .ColumnHeaders.Add , , "Description"
    .ColumnHeaders.Add , , "Created_At"
    .ColumnHeaders.Add , , "Created_By"

    .ListItems.Clear
    Do While Not RS_ADJUSTNG.EOF
    Set srcItem = .ListItems.Add(, , i, 1, 1)
        srcItem.SubItems(1) = RS_ADJUSTNG.Fields("plant_mark")
        srcItem.SubItems(2) = RS_ADJUSTNG.Fields("machine_no")
        srcItem.SubItems(3) = RS_ADJUSTNG.Fields("internal_part_id")
        srcItem.SubItems(4) = RS_ADJUSTNG.Fields("product_name")
        srcItem.SubItems(5) = RS_ADJUSTNG.Fields("period_shift")
        srcItem.SubItems(6) = RS_ADJUSTNG.Fields("period_hour")
        srcItem.SubItems(7) = RS_ADJUSTNG.Fields("ng_name")
        srcItem.SubItems(8) = RS_ADJUSTNG.Fields("adjust")
        srcItem.SubItems(9) = RS_ADJUSTNG.Fields("qty")
        srcItem.SubItems(10) = RS_ADJUSTNG.Fields("description")
        srcItem.SubItems(11) = RS_ADJUSTNG.Fields("created_at")
        srcItem.SubItems(12) = RS_ADJUSTNG.Fields("account_name")

        If RS_ADJUSTNG.AbsolutePosition >= RecordPage.PageEnd Then
            Exit Do
        Else
            RS_ADJUSTNG.MoveNext
        End If
        i = i + 1
    Loop
End With
...
Private Sub RefreshRecSum()
    lblRecSum.Caption = "Record: " & srcAdjustment & " of " & lvList.ListItems.Count
End Sub
...
On Error Resume Next
    ...
    Set RS_ADJUSTNG = New ADODB.Recordset
    If RS_ADJUSTNG.State = adStateOpen Then RS_ADJUSTNG.Close
    RS_ADJUSTNG.Open srcSQL, CN, adOpenDynamic, adLockPessimistic
