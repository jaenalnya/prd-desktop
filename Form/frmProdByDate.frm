...
Begin VB.Form frmProdByDate 
   Caption         =   "Laporan Product by Date"
   ClientHeight    =   5220
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   9450
   ...
   End
End
Attribute VB_Name = "frmProdByDate"
...
With SQLParser
    .Fields = "a.plant_mark,a.mkt_customer_id,e.name AS customer_name, a.eng_product_id,b.internal_part_id,b.name AS product_name, " & _
                "b.cavity,b.prod_yield AS target_yield,b.weight_gr,b.cycle_time_ia,a.period_shift, " & _
                "count(a.period_hour) AS jumlah_hour, " & _
                "(floor(3600/b.cycle_time_ia) * count(a.period_hour))  AS target_shot, " & _
                "sum(a.counter_ok) jumlah_shot, " & _
                "sum(a.counter_ok) * b.cavity AS gross_produksi, " & _
                "ifnull(data_ngs.jumlah_ng,0) AS jumlah_ng , " & _
                ...
                "ifnull(round(sum(a.counter_ok) / floor(3600/b.cycle_time_ia),1),0) AS runn_hours"
    ...
    .wCondition = "a.period_shift between '" & Format(DTAwal.Value, "yyyy-mm-dd") & "' and '" & Format(DTAkhir.Value, "yyyy-mm-dd") & "' and b.internal_part_id  = '" & frmDatabyProduct.lvList.SelectedItem.SubItems(2) & "' and a.plant_mark = '" & p_plant_mark & "'"

    .GroupOrder = "a.plant_mark,a.mkt_customer_id,a.eng_product_id,a.period_shift"
    .SortOrder = "a.period_shift,e.name,b.name ASC"
    .SaveStatement
End With



Set RS_PRODBYDATE = New ADODB.Recordset
If RS_PRODBYDATE.State = adStateOpen Then RS_PRODBYDATE.Close
RS_PRODBYDATE.Open SQLParser.SQLStatement, CN, adOpenDynamic, adLockPessimistic
 
 

With RecordPage
    .Start RS_PRODBYDATE, 500
End With
...
'RS_PRODBYDATE.AbsolutePosition = RecordPage.PageStart
RecordPage.PageEnd
With lvList
    .GridLines = True
    .View = lvwReport
    ...
    .ColumnHeaders.Add , , "Periode shift"
    .ColumnHeaders.Add , , "Customer name"
    .ColumnHeaders.Add , , "Internal part id"
    .ColumnHeaders.Add , , "Product name"
    .ColumnHeaders.Add , , "Jumlah shot"
    .ColumnHeaders.Add , , "Gross produksi"
    .ColumnHeaders.Add , , "Total ng"
    .ColumnHeaders.Add , , "Net produksi"
    .ColumnHeaders.Add , , "Target Yield"
    .ColumnHeaders.Add , , "Prod yield"
    .ColumnHeaders.Add , , "Persen target"
    .ColumnHeaders.Add , , "Runn hours"

    .ListItems.Clear
    Do While Not RS_PRODBYDATE.EOF
    Set srcItem = .ListItems.Add(, , i, 1, 1)
        srcItem.SubItems(1) = Format(RS_PRODBYDATE.Fields("period_shift"), "yyyy-mm-dd")
        srcItem.SubItems(2) = RS_PRODBYDATE.Fields("customer_name")
        srcItem.SubItems(3) = RS_PRODBYDATE.Fields("internal_part_id")
        srcItem.SubItems(4) = RS_PRODBYDATE.Fields("product_name")
        srcItem.SubItems(5) = RS_PRODBYDATE.Fields("jumlah_shot")
        srcItem.SubItems(6) = RS_PRODBYDATE.Fields("gross_produksi")
        srcItem.SubItems(7) = RS_PRODBYDATE.Fields("jumlah_ng")
        srcItem.SubItems(8) = RS_PRODBYDATE.Fields("net_produksi")
        srcItem.SubItems(9) = RS_PRODBYDATE.Fields("target_yield")
        srcItem.SubItems(10) = RS_PRODBYDATE.Fields("prod_yield")
        srcItem.SubItems(11) = RS_PRODBYDATE.Fields("persen_target")
        srcItem.SubItems(12) = RS_PRODBYDATE.Fields("runn_hours")

        If RS_PRODBYDATE.AbsolutePosition >= RecordPage.PageEnd Then
            Exit Do
        Else
            RS_PRODBYDATE.MoveNext
        End If
        i = i + 1
    Loop
End With
...
On Error Resume Next
    ...
    Set RS_PRODBYDATE = New ADODB.Recordset
    If RS_PRODBYDATE.State = adStateOpen Then RS_PRODBYDATE.Close
    RS_PRODBYDATE.Open srcSQL, CN, adOpenDynamic, adLockPessimistic
