...
Begin VB.Form frmLoguser 
   Caption         =   "Log Activity Users"
   ClientHeight    =   4830
   ClientLeft      =   120
   ClientTop       =   465
   ClientWidth     =   9780
   ...
   Begin lvButton.lvButtons_H cmdExport 
      ...
      Mode            =   0
      Value           =   0   'False
      Image           =   "frmLoguser.frx":4156
      cBack           =   16119285
   End
End
Attribute VB_Name = "frmLoguser"
...
'AltLVBackground lvList, vbWhite, &H8000000F, frmLoguser
...
With SQLParser
    .Fields = "*"
    .Tables = "(SELECT a.plant_mark,a.loc_code AS machine_no,b.nik,b.name AS employee_name, " & _
            "a.tr_date AS date,a.tr_time AS time,a.acc_code AS status FROM hrd_login_logs a " & _
            "LEFT JOIN hrd_employees b ON a.hrd_employee_id = b.id) Login_User"
    .wCondition = "month(date) = '" & Format(Now, "mm") & "'"
    .SortOrder = "date desc"
    .SaveStatement
End With

Set RS_RPTLOGIN = New ADODB.Recordset
If RS_RPTLOGIN.State = adStateOpen Then RS_RPTLOGIN.Close
RS_RPTLOGIN.Open SQLParser.SQLStatement, CN, adOpenDynamic, adLockPessimistic


With RecordPage
    .Start RS_RPTLOGIN, 500
End With
...
Private Sub Form_Unload(Cancel As Integer)
    Set frmLoguser = Nothing
    Set RS_RPTLOGIN = Nothing
End Sub
...
'RS_RPTLOGIN.AbsolutePosition = RecordPage.PageStart
RecordPage.PageEnd
With lvList
    .GridLines = True
    .View = lvwReport
    ...
    .ListItems.Clear
    Do While Not RS_RPTLOGIN.EOF
    Set srcItem = .ListItems.Add(, , i, 1, 1)
        srcItem.SubItems(1) = RS_RPTLOGIN.Fields("machine_no")
        srcItem.SubItems(2) = RS_RPTLOGIN.Fields("nik")
        srcItem.SubItems(3) = RS_RPTLOGIN.Fields("employee_name")
        srcItem.SubItems(4) = Format(RS_RPTLOGIN.Fields("date"), "yyyy-mm-dd")
        srcItem.SubItems(5) = Format(RS_RPTLOGIN.Fields("time"), "hh:mm:ss")
        srcItem.SubItems(6) = IIf(RS_RPTLOGIN.Fields("status") = 1, "Login", "Logout")

        If RS_RPTLOGIN.AbsolutePosition >= RecordPage.PageEnd Then
            Exit Do
        Else
            RS_RPTLOGIN.MoveNext
        End If
        i = i + 1
    Loop
End With
...
On Error Resume Next
    ...
    Set RS_RPTLOGIN = New ADODB.Recordset
    If RS_RPTLOGIN.State = adStateOpen Then RS_RPTLOGIN.Close
    RS_RPTLOGIN.Open srcSQL, CN, adOpenDynamic, adLockPessimistic
