...
Begin VB.Form frmSetupMold 
   Caption         =   "Data Setup Mold"
   ClientHeight    =   5670
   ClientLeft      =   1890
   ClientTop       =   2070
   ClientWidth     =   9615
   ...
   Begin VB.ComboBox cboMesin 
      ...
      EndProperty
      Height          =   330
      ItemData        =   "frmSetupMold.frx":0000
      Left            =   2925
      List            =   "frmSetupMold.frx":0002
   ...
   End
End
Attribute VB_Name = "frmSetupMold"
...
With SQLParser
    .Fields = "plant_mark,prod_machine_id,machine_no,machine_name,tonnage,idle_name,sum(freq) as t_freq,sum(time_minute) t_minute,round(sum(time_minute) / 60,1) as t_hours"
    .Tables = "(select a.plant_mark,a.prod_machine_id,b.number as machine_no,b.name as machine_name,b.tonnage,c.name as idle_name, " & _
        "a.prod_idletime_id,a.period_shift,count(a.prod_idletime_id) as freq,round(sum(time_to_sec(a.idle_time))/60,1) as time_minute " & _
        "from prod_machine_idles a " & _
        "left join prod_machines b on a.prod_machine_id = b.id " & _
        "left join prod_idletimes c on a.prod_idletime_id = c.id " & _
        "Where a.prod_idletime_id = 1 " & _
        "group by a.plant_mark,a.prod_machine_id,a.prod_idletime_id,a.period_shift) XX"
    ...
    .GroupOrder = "plant_mark,prod_machine_id"
...
'plant_mark,prod_machine_id,machine_no,machine_name,tonnage,idle_name,sum(freq) as t_freq,sum(time_minute) t_minute,round(sum(time_minute) / 60,1) as t_hours"
    ...
    .ColumnHeaders.Add , , "t_freq"
    .ColumnHeaders.Add , , "t_minute"
    .ColumnHeaders.Add , , "t_hours"

    .ListItems.Clear
    Do While Not RS_PRODUCT.EOF
    Set srcItem = .ListItems.Add(, , i, 1, 1)
        srcItem.SubItems(1) = RS_PRODUCT.Fields("machine_no")
        srcItem.SubItems(2) = RS_PRODUCT.Fields("tonnage")
        srcItem.SubItems(3) = RS_PRODUCT.Fields("idle_name")
        srcItem.SubItems(4) = RS_PRODUCT.Fields("t_freq")
        srcItem.SubItems(5) = RS_PRODUCT.Fields("t_minute")
        srcItem.SubItems(6) = RS_PRODUCT.Fields("t_hours")
