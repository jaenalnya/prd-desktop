...
Begin VB.Form frmRptCall 
   Caption         =   "Laporan Panggilan Mesin"
   ClientHeight    =   4560
   ClientLeft      =   120
   ClientTop       =   465
   ClientWidth     =   9555
   ...
   Begin lvButton.lvButtons_H cmdExport 
      ...
      Mode            =   0
      Value           =   0   'False
      Image           =   "frmRptCall.frx":4156
      cBack           =   16119285
   End
End
Attribute VB_Name = "frmRptCall"
...
'AltLVBackground lvList, vbWhite, &H8000000F, frmRptCall
...
With SQLParser
    .Fields = "*"
    .Tables = "(SELECT a.plant_mark,b.number AS machine_no,a.period_shift,a.start_call,a.end_call,a.idle_time,a.description " & _
                "FROM prod_call_logs a " & _
                "LEFT JOIN prod_machines b ON a.prod_machine_id = b.id) Call_Machine"
    .wCondition = "month(period_shift) = '" & Format(Now, "mm") & "'"
    .SortOrder = "period_shift desc"
    .SaveStatement
End With

Set RS_RPTCALL = New ADODB.Recordset
If RS_RPTCALL.State = adStateOpen Then RS_RPTCALL.Close
RS_RPTCALL.Open SQLParser.SQLStatement, CN, adOpenDynamic, adLockPessimistic


With RecordPage
    .Start RS_RPTCALL, 500
End With
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set frmRptCall = Nothing
    Set RS_RPTCALL = Nothing
End Sub
...
'RS_RPTCALL.AbsolutePosition = RecordPage.PageStart
RecordPage.PageEnd
With lvList
    .GridLines = True
    .View = lvwReport
    ...
    .ColumnHeaders.Add , , "start_call"
    .ColumnHeaders.Add , , "end_call"
    .ColumnHeaders.Add , , "idle_time"
    .ColumnHeaders.Add , , "description"

    .ListItems.Clear
    Do While Not RS_RPTCALL.EOF
    Set srcItem = .ListItems.Add(, , i, 1, 1)
        srcItem.SubItems(1) = RS_RPTCALL.Fields("machine_no")
        srcItem.SubItems(2) = RS_RPTCALL.Fields("period_shift")
        srcItem.SubItems(3) = Format(RS_RPTCALL.Fields("start_call"), "yyyy-mm-dd hh:mm:ss")
        srcItem.SubItems(4) = Format(RS_RPTCALL.Fields("end_call"), "yyyy-mm-dd hh:mm:ss")
        srcItem.SubItems(5) = Format(RS_RPTCALL.Fields("idle_time"), "hh:mm:ss")
        srcItem.SubItems(6) = RS_RPTCALL.Fields("description")

        If RS_RPTCALL.AbsolutePosition >= RecordPage.PageEnd Then
            Exit Do
        Else
            RS_RPTCALL.MoveNext
        End If
...
On Error Resume Next
    ...
    Set RS_RPTCALL = New ADODB.Recordset
    If RS_RPTCALL.State = adStateOpen Then RS_RPTCALL.Close
    RS_RPTCALL.Open srcSQL, CN, adOpenDynamic, adLockPessimistic
