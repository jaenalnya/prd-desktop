...
Begin VB.Form frmAdjustshot 
   Caption         =   "Data Adjustment Shot"
   ClientHeight    =   4530
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   9435
   ...
   Begin lvButton.lvButtons_H cmdNew 
      ...
      Mode            =   0
      Value           =   0   'False
      Image           =   "frmAdjustshot.frx":44F0
      cBack           =   16777215
   End
End
Attribute VB_Name = "frmAdjustshot"
...
'AltLVBackground lvList, vbWhite, &H8000000F, frmAdjustshot
...
RS_ADJUST.Open SQLParser.SQLStatement, CN, adOpenDynamic, adLockPessimistic
...
'RS_ADJUST.AbsolutePosition = RecordPage.PageStart
RecordPage.PageEnd
With lvList
    .GridLines = True
    .View = lvwReport
    ...
    .ListItems.Clear
    Do While Not RS_ADJUST.EOF
    Set srcItem = .ListItems.Add(, , i, 1, 1)
        srcItem.SubItems(1) = RS_ADJUST.Fields("plant_mark")
        srcItem.SubItems(2) = RS_ADJUST.Fields("machine_no")
        srcItem.SubItems(3) = RS_ADJUST.Fields("internal_part_id")
        srcItem.SubItems(4) = RS_ADJUST.Fields("product_name")
        srcItem.SubItems(5) = RS_ADJUST.Fields("period_shift")
        srcItem.SubItems(6) = RS_ADJUST.Fields("period_hour")
        srcItem.SubItems(7) = RS_ADJUST.Fields("adjust")
        srcItem.SubItems(8) = RS_ADJUST.Fields("qty")
        srcItem.SubItems(9) = RS_ADJUST.Fields("description")
        srcItem.SubItems(10) = RS_ADJUST.Fields("created_at")
        srcItem.SubItems(11) = RS_ADJUST.Fields("account_name")

        If RS_ADJUST.AbsolutePosition >= RecordPage.PageEnd Then
            Exit Do
        Else
            RS_ADJUST.MoveNext
        End If
...
On Error Resume Next
    ...
    Set RS_ADJUST = New ADODB.Recordset
    If RS_ADJUST.State = adStateOpen Then RS_ADJUST.Close
    RS_ADJUST.Open srcSQL, CN, adOpenDynamic, adLockPessimistic
